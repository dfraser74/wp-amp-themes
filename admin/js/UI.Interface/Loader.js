...
function WPMPMessage(){
	...
	this.view = function(params){
		...
		var no_messages = jQuery('div[id="wpmpMessageBox"]', this.parentContainer).get().length;
		
		// add message box to the parent container
		var html = '<div id="wpmpMessageBox" class="loader" style="display:none;">';
			html += 	'<div id="wpmpBoxTable" align="left" style="background: #fefcd4;">';
			html += 		'<div style="float:right; height:15px; padding-right:5px; padding-top:5px"><img id="wpmpCloseMsg" src="'+WPMPJSInterface.localpath+'admin/images/btn_close_msg.png" border="0" style="cursor:pointer" /></div>';
		...
		this.container = jQuery('div[id="wpmpMessageBox"]:eq('+no_messages+')', this.parentContainer);
		var table = jQuery('#wpmpBoxTable',this.container);
		var close_btn = jQuery('#wpmpCloseMsg',table);
...
function WPMPLoader(){
	...
	this.init = function(){
		...
		jQuery('#wpmp_messages_container').remove();
		jQuery(jQuery('body').get(0)).append('<div align="center" id="wpmp_messages_container" style="position:fixed; z-index:999999; width: 100%; display:block; top: 5px; margin: 0 auto;"></div>');
		
		var messages_container = jQuery('#wpmp_messages_container');
	...
	this.display = function(params){
		...
		var newMessage = new WPMPMessage();
